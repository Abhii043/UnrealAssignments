...
void ASelectionAreaController::SelectBoundingShape(const FString& ShapeType)
{
	...
	if (ShapeType == "Sphere") {
		if (SelectionActor) {
			...
			SelectionActor = GetWorld()->SpawnActor<ASelectionArea>(_SelectionActor, Location + FVector(0,0,Dimensions.Z/2), FRotator::ZeroRotator, SpawnParams);
		...
		if (SelectionActor) {
			...
			SelectionActor = GetWorld()->SpawnActor<ASelectionArea>(_SelectionActor, Location + FVector(0, 0, Radius), FRotator::ZeroRotator, SpawnParams);
...
{
	if (_MeshGeneratorActor) {
		MeshGeneratorActor = NewObject<AMeshGenerator>(this ,_MeshGeneratorActor);
	}
	if (MeshGeneratorActor) {
		MeshGeneratorActor->SelectedBoundingBox = SelectedBoundingBox;
		if (SelectedBoundingBox == "Box") {
			MeshGeneratorActor->Dimensions = Dimensions;
		}
		else {
			MeshGeneratorActor->Dimensions = FVector(Radius,Radius,Radius);
		}
		MeshGeneratorActor->SelectedActor = SelectionActor;
		MeshGeneratorActor->NumberOfInstances = NoOfMeshes;
